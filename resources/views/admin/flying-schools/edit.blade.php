...
@section('title', 'Edit Flying School')
@section('header', 'Edit Flying School')

@section('content')
<div class="card">
    <div class="card-body">
        <form action="{{ route('admin.flying-schools.update', $flyingSchool) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('name') is-invalid @enderror" 
                               id="name" name="name" value="{{ old('name', $flyingSchool->name) }}" required>
                        @error('name')
                        ...
                        <select class="form-select @error('country_id') is-invalid @enderror" 
                            ...
                            @foreach($countries as $country)
                                <option value="{{ $country->id }}" 
                                    {{ old('country_id', $flyingSchool->country_id) == $country->id ? 'selected' : '' }}>
                                    {{ $country->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('location') is-invalid @enderror" 
                          id="location" name="location" rows="2" required>{{ old('location', $flyingSchool->location) }}</textarea>
                @error('location')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('course_duration') is-invalid @enderror" 
                               ...
                               value="{{ old('course_duration', $flyingSchool->course_duration) }}" 
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('fleet_size') is-invalid @enderror" 
                               ...
                               value="{{ old('fleet_size', $flyingSchool->fleet_size) }}" min="0">
                        @error('fleet_size')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('flying_hours') is-invalid @enderror" 
                               id="flying_hours" name="flying_hours" 
                               value="{{ old('flying_hours', $flyingSchool->flying_hours) }}" min="0">
                        @error('flying_hours')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <select class="form-select @error('aircrafts') is-invalid @enderror" 
                    ...
                    @foreach($aircrafts as $aircraft)
                        <option value="{{ $aircraft->id }}"
                            {{ in_array($aircraft->id, old('aircrafts', $flyingSchool->aircraftIds)) ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror" 
                          id="description" name="description" rows="4">{{ old('description', $flyingSchool->description) }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('sort_order') is-invalid @enderror" 
                               id="sort_order" name="sort_order" value="{{ old('sort_order', $flyingSchool->sort_order) }}">
                        @error('sort_order')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-6">
                    <div class="mb-3 form-check pt-4">
                        <input type="checkbox" class="form-check-input" id="is_active" name="is_active" value="1" 
                               {{ old('is_active', $flyingSchool->is_active) ? 'checked' : '' }}>
            ...
            <div class="d-flex justify-content-between">
                ...
                <button type="submit" class="btn btn-primary">Update Flying School</button>
            </div>
        </form>
    </div>
</div>
